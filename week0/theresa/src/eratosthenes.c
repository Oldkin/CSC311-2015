...
// this program will identify all prime
// numbers that are less than SIZE
...
// next loop determines if i is really prime
bitmap[i] = 0;
}
// 2 is the smallest prime number so start search
// for prime numbers at 2
i = 2;
while(i < SIZE)
{
if(bitmap[i] == 0)
{
// if i is prime, then all of its multiples are 
// composite (not prime)
